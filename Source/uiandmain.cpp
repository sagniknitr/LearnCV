...
void ImageProc::readFrame()
{
    ...
    if(camstarted)
    {
        ...
        {
            capVideo.read(read_Frame);
            if(capVideo.get(CV_CAP_PROP_POS_FRAMES)==capVideo.get(CV_CAP_PROP_FRAME_COUNT) && !replayset ) //Video stream  ended
            {
                paused = true;
    ...
    if(!paused)
        originalImage = read_Frame.clone(); // frames will still be read but image processing not done
    origEmptyFlag = false;
}
...
void ImageProc::main_()
{
    ...
    updateGUI_Images(); // Update the images on the GUI .. do something else for paused
}
...
void ImageProc::updateGUI_Images()
{
    ...
    if(!processedImage[0].empty())
    {
        ...
        {
            cvtColor(processedImage[0],tempImage,CV_BGR2RGB);
        ...
        cv::resize(tempImage,tempImage,Size(ui->Image1->width()-2,ui->Image1->height()-2)); // size formatting .. -2 for the borders
        QImage Q_temp((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,imgformat); // convert to Qimage
        Q_displayedImages[0] = Q_temp;
        ui->Image1->setPixmap(QPixmap::fromImage(Q_displayedImages[0])); // Set qimage onto the label
    }
    ...
    if(!processedImage[1].empty())
    {
        ...
        {
            cvtColor(processedImage[1],tempImage,CV_BGR2RGB);
            imgformat = QImage::Format_RGB888;
        }
        ...
        cv::resize(tempImage,tempImage,Size(ui->Image2->width()-2,ui->Image2->height()-2)); // size formatting .. -2 for the borders
        QImage Q_temp((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,imgformat); // convert to Qimage
        Q_displayedImages[1] = Q_temp;
        ui->Image2->setPixmap(QPixmap::fromImage(Q_displayedImages[1])); // Set qimage onto the label
    }
    ...
    if(!processedImage[2].empty())
    {
        ...
        {
            cvtColor(processedImage[2],tempImage,CV_BGR2RGB);
            imgformat = QImage::Format_RGB888;
        }
        ...
        cv::resize(tempImage,tempImage,Size(ui->Image3->width()-2,ui->Image3->height()-2)); // size formatting .. -2 for the borders
        QImage Q_temp((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,imgformat); // convert to Qimage
        Q_displayedImages[2] = Q_temp;
        ui->Image3->setPixmap(QPixmap::fromImage(Q_displayedImages[2])); // Set qimage onto the label
    }
    ...
    if(!processedImage[3].empty())
    {
        ...
        {
            cvtColor(processedImage[3],tempImage,CV_BGR2RGB);
            imgformat = QImage::Format_RGB888;
        }
        ...
        cv::resize(tempImage,tempImage,Size(ui->Image4->width()-2,ui->Image4->height()-2)); // size formatting .. -2 for the borders
        QImage Q_temp((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,imgformat); // convert to Qimage
        Q_displayedImages[3] = Q_temp;
        ui->Image4->setPixmap(QPixmap::fromImage(Q_displayedImages[3])); // Set qimage onto the label
    }
 ...
 /*   else if(processingList[0].size() == 1)
    ...
    if(showorig)
    {
        ...
        {
            cvtColor(originalImage,tempImage,CV_BGR2RGB);
            imgformat = QImage::Format_RGB888;
        }
        ...
        cvtColor(displayedImages[0],tempImage,CV_BGR2RGB);  // opencv stores in BGR and Qt in RGB
        cv::resize(tempImage,tempImage,Size(ui->Image1->width()-2,ui->Image1->height()-2)); // size formatting .. -2 for the borders
        QImage Q_originalImage((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,imgformat); // convert to Qimage
...
/*    if(!processedImage[1].empty())
    {
        cv::resize(processedImage[1],tempImage,Size(ui->Image2->width()-2,ui->Image2->height()-2)); // size formatting .. -2 for the borders
        QImage Q_temp((uchar*)tempImage.data,tempImage.cols,tempImage.rows,tempImage.step,QImage::Format_Indexed8); // convert to Qimage
        Q_displayedImages[1] = Q_temp;
        ui->Image2->setPixmap(QPixmap::fromImage(Q_displayedImages[1])); // Set qimage onto the label
    }*/
...
void ImageProc::on_btn_StartWebcam_clicked()
{
   ...
   //  msgBox.exec();
    ...
    //QMessageBox("Success","Camera opened");
     //  msgBox.exec();
    ui->btn_StartWebcam->setText("Stop Webcam"); // once open,change the button to release capture
}
...
void ImageProc::on_btn_LoadVid_clicked()
{
    QString fileName = QFileDialog::getOpenFileName(this,
        tr("Open Video"), "C:/", tr("Video files (*.mp4 *.3gp *.avi *mkv *.flv *.divx *.mov *.wmv)"));
    if(camstarted)
    {
        capVideo.release();
        ui->btn_StartWebcam->setText("Start Webcam");
...
void ImageProc::on_btn_LoadImage_clicked()
{
    videoloaded = false;
    QString fileName = QFileDialog::getOpenFileName(this,
        tr("Open Image"), "C:/", tr("Image files (*.jpg *.jpeg *.bmp *.png)"));
    if(camstarted)
    {
        capVideo.release();
        ui->btn_StartWebcam->setText("Start Webcam");
...
void ImageProc::on_btn_Add_clicked()
{
    ...
    switch(OpBoxIndex)
    {
        case 0: ui->list_OpList1->addItem(functionList[selectedOperationIndex]);
                processingList[OpBoxIndex].push_back(selectedOperationIndex);
                addParam(OpBoxIndex, processingList[OpBoxIndex].size()-1);
                break;
        case 1: ui->list_OpList2->addItem(functionList[selectedOperationIndex]);
                processingList[OpBoxIndex].push_back(selectedOperationIndex);
                addParam(OpBoxIndex, processingList[OpBoxIndex].size()-1);
                break;
        case 2: ui->list_OpList3->addItem(functionList[selectedOperationIndex]);
                processingList[OpBoxIndex].push_back(selectedOperationIndex);
                addParam(OpBoxIndex, processingList[OpBoxIndex].size()-1);
                break;
        case 3: ui->list_OpList4->addItem(functionList[selectedOperationIndex]);
                processingList[OpBoxIndex].push_back(selectedOperationIndex);
                addParam(OpBoxIndex, processingList[OpBoxIndex].size()-1);
                break;
    default : return;
...
void ImageProc::on_btnRemove_clicked()
{
    ...
    switch(opListIndex)
    {
        case 0: ui->list_OpList1->takeItem(selectedListIndex);
                ...
                break;
        case 1: ui->list_OpList2->takeItem(selectedListIndex);
                ...
                break;
        case 2:  ui->list_OpList3->takeItem(selectedListIndex);
                ...
                break;
        case 3:  ui->list_OpList4->takeItem(selectedListIndex);
